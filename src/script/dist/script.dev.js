...
window.onload = function () {
  ...
  var tick = false;
  var scroll_direction = 'down'; //    $(window).bind('mousewheel', _.throttle(function(event) {
  //        if (tick != true) {
  //            tick = true;
  ...
  $(window).scroll(_.throttle(function () {
    if (tick != true) {
      tick = true;
      var top = $('.ecosystem').offset().top;
      var current_bottom = window.pageYOffset + document.documentElement.clientHeight;
      var current_top = window.pageYOffset;
      var st = $(this).scrollTop();

      if (st > lastScrollTop) {
        // downscroll code
        if (top + 400 > current_bottom && top > current_top) {
          $("html, body").stop().animate({
            scrollTop: top
          }, 500); //              shipBackgroundOpacity();
        ...
        // upscroll code
        if (top >= current_top && top < current_bottom) {
          $("html, body").stop().animate({
            scrollTop: 0
          }, 500);
          $("#ship_color_background, #ship_text_background").stop().animate({
...
function categoryButtonClick() {
  var control_id = this.getAttribute('aria-controls');
...
function closeMobileMenuButtonClick() {
  hideMenuBlock();
  hideMainMenu();
  hideCategoryMenu();
}
